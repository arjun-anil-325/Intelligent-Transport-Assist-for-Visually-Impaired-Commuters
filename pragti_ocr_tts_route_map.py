...
def visualize_and_categorize_boxes(thresh_roi):
    ...
    custom_config = '--oem 1 --psm 6 -l mal'
    ocr_data = pytesseract.image_to_boxes(thresh_roi, config=custom_config)
    # Assuming thresh_roi is a NumPy ndarray
    ...
    for line in ocr_data.splitlines():
        ...
        if box_height <= threshold:
            category = "small"
            small_chars += char  # Append to small_chars string
        else:
            category = "large"
            large_chars += char  # Append to large_chars string
...
def segment_ocr_text(ocr_text):
    ...
    return words
# Function to load labels and create a dictionary of unique Malayalam words
...
def write_data_to_file(file_path, ocr_text, best_matchp, conf_p,best_matchs, conf_s,ocr_match,secondary,lisst,raw_match_p):
        ...
        if any(conf > 40 for conf in confidences):  # Only proceed if confidence is high enough
            ...
            for text, match, conf, box in zip(texts, best_matches, confidences, boxes):
                if conf > 40:
                    ...
                    write_data_to_file(output_path, ocr_text , best_match_p ,best_score_p,best_match_s,best_score_s,ocr_match_p,secondary,segmented_list,raw_match_p)  # Write data to file
